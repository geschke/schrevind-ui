    ...
    <template #default>
      <div class="col-12">
        ...
        <div class="d-flex align-items-start justify-content-between flex-wrap gap-2 mb-3">
          <div>
            <h2 class="mb-1">{{ t('analyses.charts.dividends_share_by_year.title') }}</h2>
            <p class="text-muted mb-0 small">{{ t('analyses.charts.dividends_share_by_year.description') }}</p>
          </div>
        ...
        <div class="card mb-3">
          <div class="card-body py-2">
            <div class="d-flex flex-wrap align-items-end gap-3">
              <!-- Year dropdown -->
              <div>
                <div class="filter-label">{{ t('analyses.charts.dividends_share_by_year.filterYear') }}</div>
                <select
              ...
              <div class="ms-auto d-flex align-items-end gap-2">
                ...
                <div>
                  ...
                  <div class="btn-group" role="group">
                    <input type="radio" class="btn-check" id="year-share-val-gross" v-model="activeValue" value="gross" />
                    <label class="btn btn-sm btn-outline-secondary" for="year-share-val-gross">{{ t('analyses.dividends_by_year.columns.gross') }}</label>
                    <input type="radio" class="btn-check" id="year-share-val-after" v-model="activeValue" value="after_withholding" />
                    <label class="btn btn-sm btn-outline-secondary" for="year-share-val-after">{{ t('analyses.dividends_by_year.columns.after_withholding') }}</label>
                    <input type="radio" class="btn-check" id="year-share-val-net" v-model="activeValue" value="net" />
                    <label class="btn btn-sm btn-outline-secondary" for="year-share-val-net">{{ t('analyses.dividends_by_year.columns.net') }}</label>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div v-else-if="data !== null && sortedRows.length === 0" class="alert alert-info" role="alert">
          {{ t('analyses.charts.dividends_share_by_year.noData') }}
        </div>
        ...
        <template v-else-if="sortedRows.length > 0">
          ...
          <div class="card mb-3">
            <div class="card-header">
              <span class="fw-medium">
                ...
                {{ t('analyses.charts.dividends_share_by_year.chartTitle') }}
              </span>
            </div>
            <div class="card-body p-3">
              <div class="chart-wrapper">
                ...
                <div v-if="sortedRows.length > 0 && showAmounts" class="donut-center" :style="donutCenterStyle">
                  ...
                  <div class="donut-center-label">{{ t('analyses.charts.dividends_share_by_year.centerTotal') }}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="card mb-3">
            <div
              ...
              data-bs-target="#year-share-chart-options"
            >
              <span class="fw-medium">
            ...
            <div id="year-share-chart-options" class="collapse">
              <div class="card-body">
                <div class="filter-label mb-2">{{ t('analyses.charts.common.display') }}</div>
                <div class="d-flex flex-wrap gap-3">
                  <div class="form-check">
                    <input class="form-check-input" type="checkbox" id="year-share-opt-labels" v-model="showLabels" />
                    <label class="form-check-label" for="year-share-opt-labels">{{ t('analyses.charts.common.valueLabels') }}</label>
                  </div>
                  <div class="form-check">
                    <input class="form-check-input" type="checkbox" id="year-share-opt-legend" v-model="showLegend" />
                    <label class="form-check-label" for="year-share-opt-legend">{{ t('analyses.charts.common.legend') }}</label>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="card">
            <div class="card-header">
              <span class="fw-medium">
                ...
                {{ t('analyses.charts.dividends_share_by_year.tableTitle') }}
              </span>
            </div>
            <div class="card-body p-0">
              <div class="table-responsive">
                <table class="table table-hover mb-0">
                  <thead>
                    <tr>
                      <th>{{ t('analyses.charts.dividends_share_by_year.tableColSecurity') }}</th>
                      <th class="text-end">{{ t('analyses.charts.dividends_share_by_year.tableColAmount') }} ({{ currency }})</th>
                      <th class="text-end">{{ t('analyses.charts.dividends_share_by_year.tableColShare') }}</th>
                    </tr>
                  </thead>
                  <tbody>
...
import type { ShareByYearChartData, ShareByYearChartRow } from '@/types/analyses'
...
const errorMsg = ref('')
const data = ref<ShareByYearChartData | null>(null)
...
const sortedRows = computed<ShareByYearChartRow[]>(() => {
...
type ChartItem =
  | { type: 'row'; row: ShareByYearChartRow }
...
const chartItems = computed<ChartItem[]>(() => {
  ...
  let main = sorted.filter(r => getPctValue(r) >= 1.25)
  const others = sorted.filter(r => getPctValue(r) < 1.25)
...
const chartOption = computed<ECOption>(() => {
  ...
  const othersLabel = t('analyses.charts.dividends_share_by_year.others')
      ...
      formatter: (params: unknown) => {
        ...
        if (item.type === 'others') {
          return (
            ...
            `</div>` +
            `<div style="color:#868e96;font-size:11px;margin-top:6px">${t('analyses.charts.dividends_share_by_year.othersHint')}</div>`
          )
        }
        const row = item.row
        return (
...
function exportPng() {
  ...
  a.href = url
  a.download = `dividenden_anteile_${selectedYear.value}.png`
  a.click()
}
...
function loadData() {
  ...
  storeAnalyses
    .fetchShareByYearChart(selectedYear.value, ids)
    .then((result) => {
      data.value = result
    })
